...
const nextDay = (data: AppData): AppData => data.daily.date === today() ? data : {
  ...data,
  daily: { date: today(), energy: 3, mood: 3, discomfort: '无', mustDoToday: false },
  reminders: data.reminders.map((item) => item.repeat ? { ...item, done: false } : item),
};
...
const load = (): AppData => {
  try {
    ...
    return nextDay({
      ...
      diaries: parsed.diaries ?? fallback.diaries,
    });
  } catch { return defaultData(); }
};

const phaseFor = (data: AppData): SurgeryPhase => {
  const d = today(); const p = data.profile;
...
const phaseLabel: Record<SurgeryPhase, string> = { preOp: '术前准备', admission: '住院与检查', surgery: '手术窗口', recovery: '住院恢复', rest: '出院后休息' };

function planFor(data: AppData, phase: SurgeryPhase) {
  const must = data.reminders.filter((r) => r.priority === 'must' && !r.done).map((r) => r.title);
  const base = phase === 'preOp' ? ['确认入院和检查安排', '只准备最必要的住院物品'] : phase === 'admission' ? ['按通知完成检查和禁食准备', '把想问医生的问题写下来'] : phase === 'surgery' ? ['按医嘱配合，其他事先放下', '有人陪伴或能联系到即可'] : phase === 'recovery' ? ['按医嘱休息、观察与活动', '只处理必要沟通'] : ['吃饭、休息、短暂走动', '把工作和家务降到最低配'];
  const low = data.daily.energy <= 2 || data.daily.discomfort === '明显';
  return { must: [...must, ...base].slice(0, 3), defer: low ? '非紧急工作和社交可以往后放。' : '不急的事可以留到明天。', prompt: low ? '今天的标准可以低一点。先照顾身体，再谈效率。' : '身体允许时做一点点即可，不需要赶进度。' };
}
...
function App() {
  ...
  useEffect(() => {
    const refresh = () => setData((current) => nextDay(current));
    refresh();
